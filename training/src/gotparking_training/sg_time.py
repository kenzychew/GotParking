"""Singapore time (SGT) helpers shared by the training feature contract.

CRITICAL -- MUST STAY IN SYNC with ``api/_lib/sg_time.py``. This module is a
deliberate byte-for-byte copy of the serving side's SGT/holiday helper (the
CRITICAL INTEGRATION CONTRACT for T5): the training job and the batch-predict
function must resolve the exact same (dow, slot_of_day, is_holiday) tuple for
the same UTC instant, or the model trains on a feature contract that silently
drifts from what it sees at serving time -- even a one-slot drift would
corrupt the model. `tests/test_sg_time.py` cross-checks this file against
`api/_lib/sg_time.py` directly (loaded from disk via importlib, not imported
as an installed package, since api/ and training/ are separate uv projects)
across a wide range of instants, so any future edit to either file that
breaks agreement fails loudly in CI rather than silently.
...
Supabase is UTC (``timestamptz``); this module is the training-side place
that converts a UTC instant into the SGT-calendar values used as model
features (mirroring api/_lib/sg_time.py's serving-side role).
...
runtime dependency on tzdata being installed in the GitHub Actions runner,
and matches the TypeScript poller's and the Python api's equivalent helpers
exactly (all sides of the training/serving contract must agree bit-for-bit
on slot boundaries).
...
shared with the serving side per the design doc); 2027-01-01 is also
included so a training run in the last 20 minutes of 2026 SGT still resolves
the holiday feature correctly for its target time.
"""
...
# the poller's TS helper and api/_lib/sg_time.py exactly.
_SLOT_MINUTES = 15
...
# MUST STAY IN SYNC with api/_lib/sg_time.py's HOLIDAYS dict -- see module
# docstring.
# ---------------------------------------------------------------------------
HOLIDAYS: dict[date, str] = {
    ...
    # 2027 (New Year's Day only -- covers a training run's t+20min target
    # time for invocations in the last 20 minutes of 2026 SGT).
...
def sgt_parts(dt_utc: datetime) -> tuple[int, int]:
    ...
    This is the exact feature-computation contract shared with the serving
    side (api/_lib/sg_time.py): the same UTC instant must always map to the
    same (dow, slot_of_day) pair regardless of which codebase (this one, or
    the batch-predict function) computes it.

    Args:
        dt_utc: The UTC instant to convert (typically a training row's
            target time, ``base_time + 20 minutes``, per the forecast
            horizon commitment -- features describe the TARGET instant, not
            the observation instant, matching serving's call convention).
...
def is_holiday(dt_utc: datetime) -> bool:
    ...
    Args:
        dt_utc: The UTC instant to check (a training row's target time).
